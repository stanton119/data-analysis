...
model:
transfer learn from pretrained model
...
class ClassificationData(torch.utils.data.dataset.Dataset):
    ...
    def __init__(
        ...
        df["sentiment"] = df["sentiment"].apply(self._preprocess_sentiment)

        self.sentiment_map = (
            df["sentiment"].drop_duplicates().reset_index(drop=True).to_dict()
        )
        self.sentiment_map = {val: key for key, val in self.sentiment_map.items()}
        df["sentiment"] = df["sentiment"].map(self.sentiment_map)

        self.data = df

    def _preprocess_sentiment(self, sentiment: str) -> str:
    ...
    def __getitem__(self, index):
        return self.data.iloc[index]["text"], self.data.iloc[index]["sentiment"]


train_dataset = ClassificationData(filepath="data/Corona_NLP_train.csv")
test_dataset = ClassificationData(filepath="data/Corona_NLP_test.csv")
x, y = train_dataset[1]
x, y
...
trainer = pl.Trainer(
    max_epochs=10,
    callbacks=[pl.callbacks.early_stopping.EarlyStopping(monitor="val_loss", patience=30)],
)
