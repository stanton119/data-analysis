...
class SeqData(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, index):
        return lineToTensor(self.words[index]), (
            self.unique_labels.index(self.labels[index])
        )
...
dataloader_all = torch.utils.data.DataLoader(
    dataset=SeqData(
        filepath="data/names/", is_train=True, data_len=None, train_split=1
    ),
    shuffle=True,
    batch_size=1,
)
...
class RNN(pl.LightningModule):
    ...
    def forward(self, line_tensor, hidden=None):
        ...
        _all_hidden, hidden = self.rnn(line_tensor, hidden)
...
model(torch.cat([lineToTensor("James"), lineToTensor("Jones")], dim=1))
...
t1 = torch.tensor([475, 21, 2, 30, 5286])
t2 = torch.tensor([475, 21, 2, 30, 5286, 5])
torch.cat([t1, t2])
...
model = TextClassificationModel(10, 5, 4)
model

from torchinfo import summary

summary(model, input_data=(torch.tensor([1, 2, 3, 4]), torch.tensor([0, 2, 3, 4])))
