...
# %%
# predicting a single letter updates the hidden state one time.
# The output and hidden state has a fixed shape as setup with n_hidden and n_categories
# Each letter -> vector of OHE.
# Each letter is passed in one by one and updates the hidden state.
# Each time we predict we start with the hidden state = 0.
# The RNN layer takes a variable length input and condenses it down to a single hidden state vector.
...
print(output)
# %%
# Therefore running with a name rather than a character, gives the same output/hidden shape.
