...
shot_data = pd.read_csv("data/data_kobe.csv")
shot_data.head()

# %% Shot type against time
shot_data["game_date"] = pd.to_datetime(
    shot_data["game_date"], format="%Y-%m-%d"
)
one_hot = pd.get_dummies(shot_data["combined_shot_type"])
df = pd.concat([shot_data, one_hot], axis=1)
df = df.groupby("game_date").sum()
df = df.sort_index()

fig = df.rolling(200)[one_hot.columns].mean().plot(figsize=(10, 6))
...
shot_data.sort_values(
    ["game_date_int", "period", "minutes_remaining", "seconds_remaining"],
...
block_size = 50
(ln,) = plt.plot(
    [], [], marker=".", alpha=0.5, ms=1, linestyle="None", animated=True
)
...
ani = FuncAnimation(
    fig,
    update,
    frames=np.floor(shot_data.shape[0] / block_size),
    init_func=init,
    blit=True,
)
...
(ln,) = plt.plot(
    [], [], marker=".", alpha=0.9, ms=2, linestyle="None", animated=True
)
...
ani = FuncAnimation(fig, update, frames=u_years, init_func=init, blit=True)
...
df = pd.DataFrame(
    np.concatenate(
        (np.random.randint(0, high=5, size=(n, 2)), np.random.randn(n, m - 2)),
        axis=1,
    ),
...
layout.opts(opts.Scatter(color="red", tools=["hover"])).cols(1)
...
#%%
df = pd.DataFrame([10, 5], columns=["col1"])
