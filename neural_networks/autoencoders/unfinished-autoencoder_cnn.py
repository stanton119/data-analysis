...
class AutoEncoderCNN(pl.LightningModule):
    def __init__(self, n_inputs: int = 1, n_latent: int = 5):
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=1, out_channels=16, kernel_size=5, padding=2, stride=2),  # 16, 14, 14
            torch.nn.ReLU(),
            torch.nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, padding=2, stride=4),  # 32, 4, 4
            torch.nn.ReLU(),
            torch.nn.Flatten(),
            torch.nn.Linear(32*4*4, 4),
        )

        self.decoder = torch.nn.Sequential(
            torch.nn.Linear(4, 32*4*4),
            torch.nn.ReLU(),
            torch.nn.Unflatten(dim=1, unflattened_size=(32, 4, 4)),  # 32, 4, 4
            torch.nn.ConvTranspose2d(in_channels=32, out_channels=16, kernel_size=3, stride=4, padding=2, output_padding=3),
            # 16, 14, 14
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(in_channels=16, out_channels=1, kernel_size=5, stride=2, padding=2, output_padding=1),
            torch.nn.Sigmoid()
        )
...
# %%
model_cnn = AutoEncoderCNN(n_latent=10)
model_cnn.summarize()
model_dense = AutoEncoderDense(n_latent=10)
model_dense.summarize()
# %% [markdown]
...
for n_latent in latent_space_dim:
    ...
    model_cnn = AutoEncoderCNN(n_latent=n_latent)
    trainer = pl.Trainer(max_epochs=10)
    trainer.fit(model_cnn, mnist_train)
    torch.save(model_cnn, model_path / f"cnn_{n_latent}.pt")

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(model_cnn.train_log)
...
for n_latent in latent_space_dim:
    ...
    model_cnn = torch.load(model_path / f"cnn_{n_latent}.pt")
    images_all_hat = model_cnn(images_all)
    _loss = torch.nn.MSELoss()(images_all_hat, images_all)
    mse_train_cnn.append(_loss.detach().numpy())
# %% [markdown]
...
# reconstruction mse
fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(latent_space_dim, mse_train_cnn, label="cnn")
...
fig.legend()
# %% [markdown]
# We can see that the cnn autoencoder does do better generally.
...
for n_latent in latent_space_dim:
    ...
    images_all_hat = model_cnn(images_all)
    _loss = torch.nn.MSELoss()(images_all_hat, images_all)
    mse_test_cnn.append(_loss.detach().numpy())
...
# reconstruction mse
fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(latent_space_dim, mse_test_cnn, label="cnn")
...
for row, n_latent in enumerate(latent_space_dim[:4]):
    model_cnn = torch.load(model_path / f"cnn_{n_latent}.pt")
    images_hat = model_cnn(images_all).detach()
...
# Indeed we could switch the cnn layers out for convolution layers...
# 
