...
def get_model_path_from_mlflow(
    ...
    print(
        f"Looking for model with run_id={run_id}, experiment_name={experiment_name}, run_name={run_name}"
    )
    ...
    if checkpoint_artifacts:
        ...
        raise ValueError(
            f"No model artifacts or checkpoints found for run {run.info.run_id}"
        )
...
def evaluate_model(
    ...
    try:
        ...
        if model_name and feature_dim and task_names and task_types:
            ...
            if model_path.endswith(".ckpt"):
                ...
                try:
                    ...
                    if os.path.isdir(model_path) and os.path.exists(
                        os.path.join(model_path, "MLmodel")
                    ):
            ...
            raise ValueError(
                "Could not get model parameters from MLflow or data_config"
            )
    ...
    try:
            ...
            if model.model.task_types[task_name] == "binary":
                ...
                if generate_plots:
                    ...
                    roc_path = os.path.join(
                        plots_dir if plots_dir else ".", f"{task_name}_roc_curve.png"
                    )
                    ...
                    plt.plot(
                        recall,
                        precision,
                        label=f'AP = {task_metrics["average_precision"]:.4f}',
                    )
                    plt.xlabel("Recall")
                    ...
                    pr_path = os.path.join(
                        plots_dir if plots_dir else ".", f"{task_name}_pr_curve.png"
                    )
                    plt.savefig(pr_path)
                    if log_to_mlflow:
        ...
        if log_to_mlflow:
            metrics_json_path = os.path.join(
                plots_dir if plots_dir else ".", "evaluation_metrics.json"
            )
...
def evaluate_multiple_models(
    ...
    for checkpoint_file in checkpoint_files:
        ...
        plots_dir = (
            os.path.join(output_dir, model_name, "plots")
            if output_dir and generate_plots
            else None
        )
        ...
        try:
            model_metrics = evaluate_model(
                ...
                metrics=metrics,
                output_path=(
                    os.path.join(output_dir, f"{model_name}_metrics.json")
                    if output_dir
                    else None
                ),
    ...
    for model_results in results.values():
        for task_name, task_metrics in model_results.items():
            ...
            for metric_name in task_metrics:
                ...
                ):  # Skip confusion matrix for comparison
    ...
    for task_name in all_tasks:
        ...
        for metric_name in all_metrics:
            ...
            for model_name, model_results in results.items():
                if (
                    task_name in model_results
                    and metric_name in model_results[task_name]
                ):
...
def main():
    ...
    if args.model_path:
        ...
        evaluate_model(
            ...
            plots_dir=(
                os.path.join(os.path.dirname(args.output_path), "plots")
                if args.output_path
                else None
            ),
