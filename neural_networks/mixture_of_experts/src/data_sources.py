...
def load_uci_census_dataset(
    ...
    if create_multi_task:
        ...
        train_df = train_df.with_columns(
            ...
            .str.starts_with("Married")
            .cast(pl.Int8)
            .alias("marital_status_binary")
        )
        test_df = test_df.with_columns(
            pl.col("marital_status")
            .str.starts_with("Married")
...
def create_synthetic_dataset(
    ...
    if len(task_correlations) < num_tasks:
        raise ValueError(
            f"Not enough correlations provided. Need {num_tasks}, got {len(task_correlations)}"
        )
...
def generate_mmoe_synthetic_data(
    ...
    assert (
        np.abs(cos_sim - task_correlation) < 1e-10
    ), f"Expected correlation {task_correlation}, got {cos_sim}"
...
def load_correlation_experiment_datasets(
    ...
    for dataset_dir in experiment_path.iterdir():
        if dataset_dir.is_dir() and dataset_dir.name.startswith("corr_"):
            ...
            corr_str = (
                dataset_dir.name.replace("corr_", "")
                .replace("neg_", "-")
                .replace("_", ".")
            )
...
if __name__ == "__main__":
    ...
    mmoe_data = generate_mmoe_synthetic_data(
        num_samples=5000, num_features=100, task_correlation=0.5
    )
    ...
    print(
        f"Actual label correlation: {mmoe_data['metadata']['label_correlation_actual']}"
    )
    ...
    correlation_datasets = generate_correlation_experiment_datasets(
        correlations=[0.9, 0.5, 0.1], num_samples=1000, num_features=20
    )
    ...
    for corr, dataset in correlation_datasets.items():
        ...
        print(
            f"  Actual label correlation: {dataset['metadata']['label_correlation_actual']}"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print("\nLoading datasets...")
        loaded_datasets = load_correlation_experiment_datasets(
            Path(temp_dir) / "correlation_experiment"
        )
        ...
        for corr, dataset in loaded_datasets.items():
            print(f"\nCorrelation {corr}:")
            print(
                f"  Input correlation: {dataset['metadata']['task_correlation_input']}"
            )
            print(
                f"  Actual label correlation: {dataset['metadata']['label_correlation_actual']}"
            )
    ...
    regression_datasets = generate_correlation_experiment_datasets(
        correlations=[0.9, 0.5, 0.1], num_samples=1000, num_features=20, regression=True
    )
    ...
    for corr, dataset in regression_datasets.items():
        ...
        print(f"  Input correlation: {corr}")
        print(
            f"  Actual correlation: {dataset['metadata']['label_correlation_actual']}"
        )
        print(f"  Train shape: {dataset['train'].shape}")
        print(f"  Test shape: {dataset['test'].shape}")
